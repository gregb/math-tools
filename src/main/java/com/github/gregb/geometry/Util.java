...
public class Util {
	...
	public static double[] timeIntercept(final Point s1, final Point s2, final Vector v1, final Vector v2, final double radius) {
	...
	// never be in range, in which case NaN is returned for both time, or always
	// in range, in which case -Inf and +Inf are returned.
	public static double[] timeInterceptStationary(final Point s1, final Point s2, final Vector v1, final double radius) {
		...
		// r^2 = dx^2 + dy^2
		// r^2 = (l2.x - l1.x)^2 + (l2.y - l1.y)^2
		// r^2 = (s2.x - (s1.x + (t * v1.x)))^2 + (s2.y - (s1.y + (t * v1.y)))^2
		...
		// coeffecients
		final double a = v1.y * v1.y + v1.x * v1.x;
		final double b = (2 * s1.y - 2 * s2.y) * v1.y + (2 * s1.x - 2 * s2.x) * v1.x;
		final double c = s2.y * s2.y - 2 * s1.y * s2.y + s2.x * s2.x - 2 * s1.x * s2.x + s1.y * s1.y + s1.x * s1.x;
